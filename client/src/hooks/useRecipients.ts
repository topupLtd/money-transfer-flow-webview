...
export const PARENT_RECIPIENTS_KEY = ["parent-recipients"] as const;
...
/**
 * React Query hook that fetches parent recipients + pickup methods.
 *
 * Mirrors RecipientsHomeScreen.js `componentDidMount → fetchParentRecipients`.
 */
export function useParentRecipients() {
  return useQuery<ParentRecipientsData, Error>({
    queryKey: [...PARENT_RECIPIENTS_KEY],
    queryFn: fetchParentRecipients,
    staleTime: 60_000,
    gcTime: 5 * 60_000,
    retry: (failureCount, error) => {
      if ("status" in error && typeof (error as any).status === "number") {
